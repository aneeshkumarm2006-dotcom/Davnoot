...
import { SERVICE_PAGES, ADS_PAGES, TRAINING_PAGES, TOOLS_PAGES, trainingSlug } from '../lib/templates.js';
import { STATIC_URLS } from '../lib/sitemap-static.js';

const ROOT = path.join(import.meta.dirname, '..');
const NOT_FOUND_FILE = '404.html'; // generated by build.js — NOT a routable page
...
describe('vercel.json', () => {
  ...
  test('every marketing page is served at its CLEAN URL and its .html 301s to it', () => {
    ...
    for (const file of rootHtml) {
      ...
      if (TOOLS_PAGES.includes(file)) {
        // The /tools/* namespace (e.g. /tools/content-analyzer) — the same three-rule
        // shape as a service or an /ads page. The hub (tools.html) is NOT a member;
        // it is a flat page at /tools, handled by the "otherwise" branch below.
        const clean = `/tools/${name}`;
        const rdHtml = redirect(`/${file}`);
        assert.ok(rdHtml && rdHtml.destination === clean && rdHtml.permanent, `/${file} must 301 to ${clean}`);
        const rdFlat = redirect(`/${name}`);
        assert.ok(rdFlat && rdFlat.destination === clean && rdFlat.permanent, `/${name} must 301 to ${clean} (retire the flat URL)`);
...
describe('404.html — the only thing covering a no-route-matched miss', () => {
  // /services/ch shipped Vercel's grey "404: NOT_FOUND" panel once. It is TWO
  // segments, so the /:slug catch-all (single-segment) never matched it and
  // api/page.js never ran. The static 404.html is the fallback Vercel serves when
  // nothing at all matches, and it is the ONLY layer that covers that shape.
  const file = path.join(ROOT, NOT_FOUND_FILE);

  test('exists on disk and is byte-identical to lib/not-found.js', () => {
    assert.ok(fs.existsSync(file), 'run `npm run site` — build.js generates it');
    assert.equal(
      fs.readFileSync(file, 'utf8'),
      renderNotFound(),
      '404.html has drifted from its renderer — run `npm run site`, do not hand-edit it',
    );
  });

  test('is noindex (it 200s at its own /404.html path — a soft 404 Google could index)', () => {
    assert.match(fs.readFileSync(file, 'utf8'), /<meta name="robots" content="noindex, follow" \/>/);
  });

  test('carries the site chrome and a route back in', () => {
  ...
  test('has NO redirect or rewrite of its own', () => {
    // A rewrite would make /404 a real 200 URL; a redirect would fire before the
    // static file and turn every miss into a redirect chain.
    const sources = [...(config.redirects || []), ...(config.rewrites || [])].map((r) => r.source);
    for (const s of ['/404', '/404.html']) {
      assert.ok(!sources.includes(s), `${s} must have no vercel.json rule — 404.html is served by the fallback, not routed`);
    }
  });

  test('is not in the sitemap', () => {
    const locs = STATIC_URLS.map((u) => u.loc);
    assert.ok(!locs.some((l) => /\/404(\.html)?$/.test(l)), `404 must never be sitemapped: ${locs.join(', ')}`);
  });

  test('the 410 variant is the same page with retired-page copy', () => {
    const gone = renderNotFound({ status: 410 });
    assert.match(gone, /410/, 'the 410 body should say so');
    assert.match(gone, /<meta name="robots" content="noindex, follow" \/>/);
    assert.notEqual(gone, renderNotFound({ status: 404 }), '404 and 410 must not read identically');
  });
});
