...
describe('sitemap.xml — the migration safety net', () => {
  ...
  test('lib/sitemap-static.js is current (== root .html minus CMS pages)', () => {
    ...
    // 404.html is generated by build.js and is noindex — it is a fallback body, not
    // a page, and sitemapping it would submit a URL that answers "not found".
    const isGenerated = (f) => f === '404.html';
    const rootHtml = fs
      .readdirSync(REPO)
      .filter((f) => f.endsWith('.html') && !isVerification(f) && !isGenerated(f));
