 ...
 */
import { SITE_URL, FAVICON_TAGS, ANALYTICS_TAGS, esc, navHtml, footerHtml, jsonLdSafe, canonicalFor } from './templates.js';
...
function head(content, url, settings, lang) {
  ...
  const ogImage = seo.ogImage || settings.defaults?.ogImage || SITE_URL + '/images/davnoot-logo.png';
...
${navHtml('composed', '/', s)}
  ...
  ${/* <main> is the page's content landmark — the thing a screen reader's "skip to
       content" jumps to, and one of the semantic-HTML signals a composed page had
       none of. Every hand-written marketing page now carries the same wrapper, so a
       page built in /admin must not be the one shape that doesn't. */ ''}
  <main id="main">
${body}
  </main>
