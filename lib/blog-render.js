...
import { normalizeLinkRel } from './link-rel.js';
...
export function renderArticlePage(post, { preview = false, related = [] } = {}) {
  ...
  //
  // ORDER MATTERS. normalizeLinkRel runs on the STORED body first, so it only ever
  // sees links the author (or, historically, the editor's TipTap defaults) put
  // there. injectKeywordLinks then adds its own anchors, which already emit correct
  // rel from the backlink row's setting — normalizing those again would be a no-op
  // at best and would erase a deliberate rel=sponsored at worst.
  const body = injectKeywordLinks(
    normalizeLinkRel(post.content || '', { siteHost: SITE_HOST }),
    post.keywords || [],
    {
  ...
  const content = `
${banner}
${/* The post page had no content landmark: <nav>, then <article>, then <footer>, with
     nothing marking where the page's own content starts. /blog and every marketing
     page carry <main>, so the one page type people actually land on from search had
     the weakest document outline of the three. The preview banner stays OUTSIDE it —
     it is chrome for the author, not part of the article. */ ''}
<main id="main">
...
<section class="post-cta">
  ...
  <a href="/book-call" class="btn-primary" data-cursor>Book a call →</a>
</section>
</main>
...
function renderRelated(related) {
  ...
  <div class="post-grid">
    ${/* NOT .map(postCard) — Array.map passes the INDEX as the 2nd argument, which
         lands in postCard's catNames slot and makes it call (0).get(). That threw a
         TypeError on every post whose related rail contained a categorised post, i.e.
         a 500 on the whole public blog. Always wrap a multi-arg fn passed to .map. */ ''}
...
export function postCard(post, catNames) {
  ...
  // The typeof guard is deliberate: this function is one .map(postCard) away from
  // being handed an index instead of a Map, and a card must degrade to its tag chip
  // rather than 500 the page that embeds it.
  const primarySlug = post.categories?.[0];
  const lookup = typeof catNames?.get === 'function' ? catNames : null;
  const label = (primarySlug && lookup?.get(primarySlug)) || post.tags?.[0] || '';
...
/* ---------------------------------------------------------- category archives --
 *
 * A category archive used to be an <h1>, a one-line generated subhead, and a grid of
 * cards. Semrush (2026-08-01) flagged all six of them at once for "Low word count",
 * and it was right: a page whose only unique text is its own title is a page Google
 * has no reason to rank and every reason to treat as a near-duplicate of the five
 * beside it.
 *
 * Two things fix that, in this order:
 *   1. the category's own `description` — real, human-written copy, edited in
 *      /seoteam → Categories. This is the one that actually differentiates the page.
 *   2. this block — a genuine "where to go next" section. It is NOT filler: every
 *      line is a link a reader on a topic archive plausibly wants, and it gives each
 *      archive outbound internal links it previously had none of (they were leaf
 *      nodes hanging off /blog).
 *
 * Rule for anything added here later: it must be USEFUL to a reader on that page. A
 * paragraph of keyword mush that exists to clear a word-count threshold is exactly
 * the thing the December 2025 core update demotes, and it would trade a "Low word
 * count" notice for a much more expensive problem.
 */
function renderArchiveIntro(activeCategory) {
  const desc = activeCategory?.description?.trim();
  if (!desc) return '';
  return `<div class="archive-intro">
  ${desc
    .split(/\n{2,}/)
...
/* The services a topic archive should hand a reader off to. Keyed by a substring of
 * the category slug so a renamed-but-not-re-slugged category keeps matching, with a
 * site-wide default for anything unrecognised. */
const ARCHIVE_LINKS = [
  { match: /ai|answer|geo/, links: [['/services/ai-seo', 'AI search optimization'], ['/services/seo', 'SEO'], ['/services/chatgpt-ads', 'ChatGPT Ads']] },
  { match: /seo|search/, links: [['/services/seo', 'SEO'], ['/services/shopify-seo', 'Shopify SEO'], ['/services/ai-seo', 'AI search optimization']] },
  { match: /content/, links: [['/services/seo', 'SEO'], ['/services/email', 'Email marketing'], ['/services/ai-seo', 'AI search optimization']] },
  { match: /software|develop|web/, links: [['/services/software', 'Custom software'], ['/services/shopify', 'Shopify development'], ['/services/seo', 'SEO']] },
  { match: /analytic|growth|business/, links: [['/services/google-ads', 'Google Ads'], ['/services/meta-ads', 'Meta Ads'], ['/services/email', 'Email marketing']] },
  { match: /ads|paid|media|social/, links: [['/services/google-ads', 'Google Ads'], ['/services/meta-ads', 'Meta Ads'], ['/ads', 'All ad platforms']] },
];
const DEFAULT_ARCHIVE_LINKS = [['/services', 'All services'], ['/services/seo', 'SEO'], ['/services/google-ads', 'Google Ads']];

function renderArchiveFooter(activeCategory, categories) {
  if (!activeCategory) return '';

  const slug = activeCategory.slug || '';
  const services = (ARCHIVE_LINKS.find((r) => r.match.test(slug)) || {}).links || DEFAULT_ARCHIVE_LINKS;

  // Sibling archives, so each topic page is one hop from every other one instead of
  // only from /blog. Capped at six: a link list longer than the card grid above it
  // stops reading as navigation and starts reading as a footer.
  const siblings = (categories || []).filter((c) => c.slug !== slug).slice(0, 6);

  return `<aside class="archive-more">
  <div class="archive-more-col">
    <h2>Work with us on ${esc(activeCategory.name)}</h2>
    <p>Reading is the cheap part. If any of this is a live problem rather than a curiosity, these are the services that touch it — or skip the reading and <a href="/book-call">book a strategy call</a>.</p>
    <ul>
      ${services.map(([href, label]) => `<li><a href="${esc(href)}">${esc(label)}</a></li>`).join('\n      ')}
    </ul>
  </div>
  ${
    siblings.length
    ...
    <h2>Other topics</h2>
    <ul>
      ${siblings.map((c) => `<li><a href="/blog/category/${esc(c.slug)}">${esc(c.name)}</a></li>`).join('\n      ')}
    </ul>
    <p><a href="/blog">Browse every post →</a></p>
  </div>`
      : ''
  }
</aside>`;
...
export function renderIndexPage({ posts: list, page = 1, totalPages = 1, categories = [], activeCategory = null }) {
  ...
  /* The archive's own copy makes a far better meta description than the generated
   * sentence — it is the one string on the page that is unique to this category.
   * Trimmed to ~160 chars on a word boundary so the SERP snippet isn't cut mid-word. */
  const clamp = (s, n = 158) => {
  ...
  const desc = activeCategory
    ? activeCategory.description?.trim()
      ? clamp(activeCategory.description)
  ...
  /* The archive graph goes through the SAME builder as everything else. It used
   * to be hand-assembled here, which made it a second, drifting JSON-LD builder
   * whose nodes had no @id. The crumbs/items are the archive's own data; the node
   * shapes belong to lib/structured-data.js. The trailing crumb carries no href
   * so the builder resolves it to this page's canonical. */
  const crumbs = [
    { name: 'Home', href: SITE_URL + '/' },
    activeCategory ? { name: 'Blog', href: BLOG_BASE } : { name: 'Blog', href: null },
  ];
  if (activeCategory) crumbs.push({ name: activeCategory.name, href: null });

  const jsonLd = jsonLdSafe({
    '@context': 'https://schema.org',
    '@graph': buildGraph(
      { crumbs, items: list.map((p) => ({ url: postUrl(p), name: p.title })) },
      canonical,
      'blogArchive',
    ),
  });
  ...
  /* A category archive with nothing in it is a real page (200, not 404 — the category
   * exists and gets posts the moment one is filed under it), but it has zero content.
   * Leaving it "index, follow" hands Google a soft 404, and /blog links a pill to every
   * category, so all of them get crawled. noindex,follow keeps the crawl path open while
   * keeping the empty shell out of the index; it flips back automatically on the next
   * render once a post is filed. api/blog/index.js already 404s a category that does
   * not exist — this is the other half of that rule.
   *
   * The unfiltered /blog is deliberately NOT covered: an empty blog is a launch state,
   * not a soft 404, and de-indexing /blog itself would be far worse than a thin page. */
  const emptyArchive = Boolean(activeCategory) && !list.length;

  const head = [
    `<title>${esc(title)}</title>`,
    `<meta name="description" content="${esc(desc)}" />`,
    `<link rel="canonical" href="${esc(canonical)}" />`,
    `<meta name="robots" content="${emptyArchive ? 'noindex, follow' : 'index, follow'}" />`,
    ...FAVICON_TAGS,
  ...
  <p>We're writing. Check back shortly — or <a href="/book-call">book a call</a> and skip the reading.</p>
</div>`;
  ...
  const breadcrumbNav = activeCategory
    ? `<a href="/">Home</a> <span>/</span> <a href="/blog">Blog</a> <span>/</span> <span aria-current="page">${esc(activeCategory.name)}</span>`
...
<header class="blog-hero">
  ...
  ${heroHeading}
  ${heroSub}
  ${renderArchiveIntro(activeCategory)}
</header>
...
<main id="main" class="blog-index">
  ...
  ${renderArchiveFooter(activeCategory, categories)}
</main>`;
 ...
 *
 * The numbers are not decoration. With prev+next only, page 2 was reachable from
 * exactly ONE page on the whole site — /blog — which is what Semrush reported as
 * "Pages with only one internal link", and it gets worse as the blog grows: page 5
 * would be four hops deep from the index, at the bottom of a crawler's priority list.
 * Numbered links make every page of every archive at most two hops from /blog.
 *
 * The window is deliberately small (current ± 2, with first/last always pinned and a
 * gap marker between): a full 1..N list on a blog that eventually has 40 pages is a
 * link farm in its own footer, and "Too many on-page links" is the next audit item
 * you'd trade this one for.
 */
function pageWindow(page, totalPages, span = 2) {
  const want = new Set([1, totalPages]);
  for (let p = page - span; p <= page + span; p++) if (p >= 1 && p <= totalPages) want.add(p);

  const out = [];
  let previous = 0;
  for (const p of [...want].sort((a, b) => a - b)) {
    if (previous && p - previous > 1) out.push(null); // -> the … gap marker
    out.push(p);
    previous = p;
  }
  return out;
    ...
    .map((p) =>
      p === null
        ...
        : p === page
          ...
          : `<a class="page-num" href="${esc(href(p))}" aria-label="Page ${p}">${p}</a>`,
    )
    .join('\n    ');

  return `<nav class="blog-pagination" aria-label="Pagination">
  ${page > 1 ? `<a class="page-step" href="${esc(href(page - 1))}" rel="prev">← Newer</a>` : '<span class="page-step is-off"></span>'}
  <span class="page-nums">
    ${numbers}
  </span>
  ${page < totalPages ? `<a class="page-step" href="${esc(href(page + 1))}" rel="next">Older →</a>` : '<span class="page-step is-off"></span>'}
</nav>`;
}
