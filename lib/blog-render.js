...
export function postCard(post) {
  ...
  <div class="card-body">
    ${post.tags?.[0] ? `<span class="card-tag">${esc(post.tags[0])}</span>` : ''}
...
/** Render the /blog index (paginated). */
export function renderIndexPage({ posts: list, page = 1, totalPages = 1 }) {
  const canonical = page > 1 ? `${BLOG_BASE}?page=${page}` : BLOG_BASE;
  const title = page > 1 ? `Blog — Page ${page} — Davnoot` : 'Blog — Growth, SEO & AI Search Insights — Davnoot';
  const desc =
  ...
  const head = [
    ...
    page > 1 ? `<link rel="prev" href="${page - 1 === 1 ? BLOG_BASE : `${BLOG_BASE}?page=${page - 1}`}" />` : '',
    page < totalPages ? `<link rel="next" href="${BLOG_BASE}?page=${page + 1}" />` : '',
    `<script type="application/ld+json">\n${jsonLd}\n</script>`,
  ]
    .filter(Boolean)
    .join('\n');

  const empty = `<div class="blog-empty">
...
<main class="blog-index">
  ${list.length ? `<div class="post-grid">${list.map(postCard).join('\n')}</div>` : empty}
...
function renderPagination(page, totalPages) {
  ...
  const href = (p) => (p === 1 ? '/blog' : `/blog?page=${p}`);
  return `<nav class="blog-pagination" aria-label="Pagination">
  ${page > 1 ? `<a href="${href(page - 1)}" rel="prev">← Newer</a>` : '<span></span>'}
  <span class="page-count">Page ${page} of ${totalPages}</span>
  ${page < totalPages ? `<a href="${href(page + 1)}" rel="next">Older →</a>` : '<span></span>'}
</nav>`;
}
