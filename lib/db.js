...
/** Blog categories — the managed taxonomy behind the /blog filter pills. */
...
/** Booking-form submission throttle — one doc per attempt, expired by a TTL index.
 *  Rate limiting lives in Mongo (not module memory) for the same reason the login
 *  throttle does: serverless scales out, so an in-process counter is trivially
 *  bypassed. Keyed on IP and email so a bot flood is capped either way. */
export async function formAttempts() {
  return (await getDb()).collection('form_attempts');
}

/** Submissions the classifier rejected outright, held for 30 days by a TTL index.
 *
 *  A hard reject never reaches `leads`, which is the point — but a filter with no
 *  bin behind it turns a false positive into a destroyed prospect, and "never lose
 *  a lead" is the rule this whole subsystem exists to protect. So every rejection
 *  is written here with the score and reasons that produced it, readable in
 *  /admin under Leads → Blocked, and swept automatically a month later. */
export async function blockedSubmissions() {
  return (await getDb()).collection('blocked_submissions');
}
 ...
 */
/* Create or RETUNE a TTL index on `at`.
 *
 * createIndex is idempotent only while the options match. Re-running it with a
 * different expireAfterSeconds fails with IndexOptionsConflict (85) against an
 * index that already exists — which is exactly what happens the first time this
 * script runs after a retention change, on a collection full of live documents.
 * Dropping and recreating would be the obvious fix and is wrong: between the two
 * calls the collection has no TTL at all. collMod retunes it in place.
 */
async function ttlIndex(col, name, seconds) {
  try {
    await col.createIndex({ at: 1 }, { name, expireAfterSeconds: seconds });
  } catch (err) {
    if (err?.code !== 85 && err?.codeName !== 'IndexOptionsConflict') throw err;
    const db = await getDb();
    await db.command({ collMod: col.collectionName, index: { name, expireAfterSeconds: seconds } });
  }
}
...
export async function ensureIndexes() {
  ...
  // Powers the public category archive (/blog/category/:slug) — a post carries an
  // array of category SLUGS, so this multikey index makes the filtered list query
  // ({ categories: slug }) an index scan rather than a collection sweep.
  await p.createIndex({ categories: 1, publishedAt: -1 }, { name: 'categories_publishedAt' });
  await p.createIndex(
    { title: 'text', excerpt: 'text', tags: 'text' },
    { name: 'post_search', weights: { title: 10, excerpt: 4, tags: 2 } },
  );

  // Categories — `slug` is the natural key and the archive URL, so it is unique.
  // Deleting a category also $pulls its slug from every post (see the DELETE
  // handler); this index keeps that lookup and the create-time collision check fast.
  const cat = await categories();
  await cat.createIndex({ slug: 1 }, { unique: true, name: 'category_slug_unique' });
  await cat.createIndex({ name: 1 }, { name: 'category_name' });
  ...
  // Booking-form throttle. The compound indexes make each count an index scan.
  //
  // Retention is 24h rather than the old 60m because the duplicate-content check
  // needs a full day of history: the 2026-08 flood replayed ONE payload from a
  // dozen rented IPs over 36 hours, and a one-hour memory cannot see that. The
  // per-IP and per-email limits still use their own 10-minute window — they filter
  // on `at` explicitly, so retention and window are independent knobs.
  const fa = await formAttempts();
  await ttlIndex(fa, 'form_attempt_ttl', 24 * 60 * 60);
  await fa.createIndex({ ip: 1, at: -1 }, { name: 'form_ip_at' });
  await fa.createIndex({ email: 1, at: -1 }, { name: 'form_email_at' });
  // The neighbourhood throttle: one rented /24 supplying many addresses is the
  // shape a proxy rotation makes, and is invisible to a per-address limit.
  await fa.createIndex({ prefix: 1, at: -1 }, { name: 'form_prefix_at' });
  await fa.createIndex({ hash: 1, at: -1 }, { name: 'form_hash_at' });

  // Rejected submissions, swept after 30 days.
  const bs = await blockedSubmissions();
  await ttlIndex(bs, 'blocked_ttl', 30 * 24 * 60 * 60);
  await bs.createIndex({ at: -1 }, { name: 'blocked_at' });
  ...
  // The inbox's default view is "not spam, newest first" — the one query that
  // runs on every /admin page load.
  await l.createIndex({ spam: 1, createdAt: -1 }, { name: 'leads_spam_created' });
