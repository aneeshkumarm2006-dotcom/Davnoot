...
export async function ensureIndexes() {
  ...
  // Booking-form throttle. TTL expires each attempt after the rate-limit window,
  // and the compound indexes make the per-IP / per-email count a quick index scan.
  const fa = await formAttempts();
  await fa.createIndex({ at: 1 }, { name: 'form_attempt_ttl', expireAfterSeconds: 60 * 60 });
