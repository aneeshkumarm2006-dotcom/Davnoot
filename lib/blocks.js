...
const RENDERERS = {
  // A richtext block comes out of the SAME Tiptap surface as the main body, so it
  // carries the same rel="…nofollow" artefact on every link the toolbar inserted —
  // including links to our own pages. Normalized here for the same reason and by the
  // same rules as the body (see lib/link-rel.js). htmlEmbed deliberately is NOT:
  // that block is raw markup the author pasted verbatim, so a rel there is theirs.
  richtext: (d) => `<div class="blk blk-richtext">${normalizeLinkRel(sanitizeBody(d.html))}</div>`,
