...
/**
 * Is this post document visible to the world right now? The IN-MEMORY twin of
 * publishedFilter, for callers that hold the document rather than a query — a
 * write route deciding whether a save just made a URL appear or disappear.
 *
 * It must stay in lockstep with the filter above: a post scheduled into the
 * future is NOT live, which is the rule a hand-rolled `status === 'published'`
 * check always gets wrong. (lib/page-model.js's isPageLive is the same idea for
 * pages, and exists for the same reason.)
 */
export function isPostLive(post, now = new Date()) {
