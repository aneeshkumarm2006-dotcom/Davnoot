...
export const CONFIG = {
  ...
  /* A free public tool (/tools/*). SoftwareApplication rather than Service, because
   * nothing here is sold: it has no price, no provider relationship, and no sales
   * motion, and a Service node would describe an engagement that does not exist.
   *
   * Google's SoftwareApplication rich result needs one of offers, aggregateRating,
   * or review. We emit `offers` at price 0 — the honest one. We will never emit
   * aggregateRating or review here: Google is explicit that ratings an entity
   * controls about itself are ineligible, which is the same rule that keeps the
   * homepage testimonials out of the JSON-LD (see the note in davnoot-schema-engine). */
  tool: { nodes: ['Organization', 'SoftwareApplication', 'BreadcrumbList', 'FAQPage'] },
};
...
const BUILDERS = {
  ...
  /* The free-tool node. Everything it asserts is verifiable on the page itself:
   * the price really is zero, it really does run in the browser, and every entry in
   * featureList is a check the tool visibly performs. Structured data that claims
   * more than the page shows is the one kind that actually costs you something.
   *
   * `browserRequirements` is not decoration — the tool is client-side JavaScript, so
   * a consumer that renders without JS gets nothing, and saying so is accurate.
   * A tool with no TOOL_META row still emits a valid node: the name falls back to the
   * page title with the brand suffix stripped, exactly as the Service builder does. */
  SoftwareApplication: (doc, ctx) => {
    const meta = TOOL_META[doc.file] || {};
    return {
      '@type': 'SoftwareApplication',
      '@id': ctx.url + '#app',
      name: meta.name || String(doc.title || '').replace(/\s*[—|]\s*Davnoot\s*$/, ''),
      url: ctx.url,
      description: doc.desc,
      applicationCategory: meta.category || 'BusinessApplication',
      applicationSubCategory: meta.subCategory,
      operatingSystem: 'Any (modern web browser)',
      browserRequirements: 'Requires JavaScript.',
      inLanguage: doc.locale === 'fr' ? 'fr-CA' : 'en',
