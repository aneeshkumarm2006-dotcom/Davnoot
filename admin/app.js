/*srchash:71d298018f9849f8ba971c29ac4b292cc7b776b38fae9593872551e47397e6fe*/
(()=>{var a=s=>String(s??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"),P=(s,e=document)=>e.querySelector(s);function _(s){if(!s)return"\u2014";let e=new Date(s);return Number.isNaN(e.getTime())?"\u2014":e.toLocaleDateString("en-CA",{year:"numeric",month:"short",day:"numeric"})}function M(s){if(!s)return"\u2014";let e=new Date(s);return Number.isNaN(e.getTime())?"\u2014":e.toLocaleDateString("en-US",{timeZone:"America/Toronto",month:"short",day:"numeric",year:"numeric"})}function N(s){if(!s)return"";let e=new Date(s);return Number.isNaN(e.getTime())?"":e.toLocaleTimeString("en-US",{timeZone:"America/Toronto",hour:"numeric",minute:"2-digit",hour12:!0})}function k(s){if(!s)return"\u2014";let e=new Date(s),t=Math.round((Date.now()-e.getTime())/6e4);if(t<1)return"just now";if(t<60)return`${t}m ago`;let i=Math.round(t/60);if(i<24)return`${i}h ago`;let n=Math.round(i/24);return n<30?`${n}d ago`:_(s)}function u(s,{confirmLabel:e="Confirm",danger:t=!1}={}){return new Promise(i=>{let n=document.createElement("div");n.className="modal-backdrop",n.innerHTML=`
      ...
      </div>`;let o=r=>{n.remove(),document.removeEventListener("keydown",d),i(r)},d=r=>{r.key==="Escape"&&o(!1)};n.addEventListener("click",r=>{(r.target===n||r.target.dataset.act==="cancel")&&o(!1),r.target.dataset.act==="ok"&&o(!0)}),document.addEventListener("keydown",d),document.body.appendChild(n),n.querySelector('[data-act="ok"]').focus()})}function l(s,e="ok"){let t=document.createElement("div");t.className=`toast is-${e}`,t.textContent=s,document.body.appendChild(t),setTimeout(()=>t.classList.add("in"),10),setTimeout(()=>{t.classList.remove("in"),setTimeout(()=>t.remove(),300)},3200)}var S=class extends Error{constructor(e,t,i){super(t),this.status=e,this.fields=i||{}}};async function h(s,e,t,i){let n=await fetch(e,{method:s,headers:{...t?{"Content-Type":"application/json"}:{},...i||{}},body:t?JSON.stringify(t):void 0,credentials:"same-origin"});if(n.status===401){let d=encodeURIComponent(location.pathname+location.search);throw location.href=`/seoteam/login?next=${d}`,new S(401,"Signed out.")}let o=null;try{o=await n.json()}catch{}if(!n.ok)throw new S(n.status,o?.error||`Request failed (${n.status})`,o?.fields);return o}var c={overview:()=>h("GET","/api/admin/overview"),listPages:()=>h("GET","/api/admin/pages"),createPage:s=>h("POST","/api/admin/pages",s),getPage:s=>h("GET",`/api/admin/pages/${encodeURIComponent(s)}`),savePageDraft:(s,e,t)=>h("PUT",`/api/admin/pages/${encodeURIComponent(s)}`,e,t!=null?{"If-Match":String(t)}:null),publishPage:(s,e)=>h("POST",`/api/admin/pages/${encodeURIComponent(s)}/publish`,e||{}),deletePage:s=>h("DELETE",`/api/admin/pages/${encodeURIComponent(s)}`),pageRevisions:s=>h("GET",`/api/admin/pages/${encodeURIComponent(s)}/revisions`),restoreRevision:(s,e)=>h("POST",`/api/admin/pages/${encodeURIComponent(s)}/revisions`,{version:e}),seoTable:()=>h("GET","/api/admin/seo"),patchSeo:s=>h("PATCH","/api/admin/seo",s),listLeads:()=>h("GET","/api/admin/leads"),patchLead:s=>h("PATCH","/api/admin/leads",s),listRedirects:()=>h("GET","/api/admin/redirects"),createRedirect:s=>h("POST","/api/admin/redirects",s),deleteRedirect:s=>h("DELETE",`/api/admin/redirects?source=${encodeURIComponent(s)}`),getSettings:()=>h("GET","/api/admin/settings"),saveSettings:s=>h("PUT","/api/admin/settings",s),audit:()=>h("GET","/api/admin/audit"),logout:()=>h("POST","/api/seoteam/logout",{})};var L=class{constructor(e){this.root=e}async mount(){this.root.innerHTML='<div class="loading">Loading\u2026</div>';let e;try{e=await c.overview()}catch(t){this.root.innerHTML=`<div class="empty"><h2>Couldn't load the overview</h2><p class="muted">${a(t.message)}</p></div>`;return}this.render(e)}render(e){this.root.innerHTML=`
      ...
      <div class="ov-grid">
        ${b(e.pages.total,"Pages",`${e.pages.marketing} marketing \xB7 ${e.pages.composed} custom`)}
        ${b(e.pages.unpublishedDrafts,"Unpublished drafts","pages with pending edits")}
        ${b(e.posts.published,"Blog posts",`${e.posts.draft} drafts`)}
        ${b(e.leads.unread,"New leads",e.leads.unread?"awaiting a reply":"inbox clear",e.leads.unread?"warn":"")}
        ${b(e.media.total,"Media","in the library")}
      </div>
    ...
    `}table(e,t){return`<div class="table-scroll"><table class="grid-table">
      ...
      <tbody>${e.map(i=>`
        <tr>
          ...
          <td><span class="pill pill-${O(i.status)}">${a(i.status)}</span></td>
...
${t}`,{confirmLabel:"Restore latest"}))try{await c.restoreRevision(this.key,e.revisions[0].version),l("Restored into draft."),this.mount()}catch(i){l(i.message,"err")}}reloadPreview(){clearTimeout(this._pv),this._pv=setTimeout(()=>{let e=this.root.querySelector("#preview");e&&(e.src=e.src)},400)}setStatus(e,t){let i=this.root.querySelector("#status");i&&(i.textContent=e,i.className="editor-status "+(t||""))}renderComposed(){this.root.innerHTML=`
      ...
      <div class="editor-split">
        <div class="editor-fields">
          <div class="section-palette">
            ...
            ${B.map(e=>`<button class="btn btn-ghost btn-add-section" data-type="${e}">+ ${a(y[e].label)}</button>`).join("")}
          </div>
      ...
      </div>`,this.wireComposed()}sectionCard(e,t){let i=y[e.type];if(!i)return"";let n=i.fields.map(d=>this.fieldControl(`s${t}.${d.key}`,d,e.fields?.[d.key])).join(""),o=i.item?`<div class="section-items"><div class="muted small" style="margin:8px 0 4px">${a(i.item.label)}s</div>
          ${(e.items||[]).map((d,r)=>`<div class="section-item">
            <div class="section-item-head"><span class="muted small">${a(i.item.label)} ${r+1}</span><button class="btn-icon btn-del-item" data-s="${t}" data-i="${r}" title="Remove">\u2715</button></div>
            ${i.item.fields.map(p=>this.fieldControl(`s${t}.i${r}.${p.key}`,p,d?.[p.key])).join("")}
          </div>`).join("")}
...
`).map(j=>j.trim()).filter(Boolean):p=e.value,o!=null?((f=r.items||(r.items=[]))[q=+o]||(f[q]={}),r.items[+o][d]=p):(r.fields||(r.fields={}))[d]=p}touchComposed(){this.dirty=!0,this.setStatus("Unsaved\u2026","dirty"),clearTimeout(this._debounce),this._debounce=setTimeout(()=>this.saveComposed(),900)}async saveComposed(){if(!this.saving){this.saving=!0,this.setStatus("Saving\u2026","dirty");try{let e={title:this.page.draft?.title||this.key,sections:this.sections},t=await c.savePageDraft(this.key,e,this.page.version);this.page.version=t.version,this.dirty=!1,this.setStatus("Saved to draft","saved"),this.reloadPreview()}catch(e){e.status===409?(this.setStatus("Someone else saved \u2014 reload","dirty"),l("This page changed elsewhere. Reload to continue.","err")):(this.setStatus("Save failed","dirty"),l(e.message,"err"))}finally{this.saving=!1}}}renderNew(){this.root.innerHTML=`
      ...
      <div class="table-controls" style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px">
        ${H.map(t=>`<button class="btn btn-sm ${this.filter===t.id?"btn-dark":"btn-ghost"}" data-filter="${t.id}">${a(t.label)}</button>`).join("")}
      </div>
    ...
    `}row(e){let t=`${e.type}:${e.key}`;return`<tr data-ref="${a(t)}">
    ...
    </tr>`}wire(){this.root.querySelectorAll("[data-filter]").forEach(e=>e.addEventListener("click",()=>{this.filter=e.dataset.filter,this.render(),this.wire()})),this.root.querySelectorAll("tr[data-ref]").forEach(e=>{let[t,i]=e.dataset.ref.split(/:(.+)/);e.querySelectorAll("[data-field]").forEach(n=>{let o=async()=>{let d=n.dataset.field,r=n.value;d==="seo.robotsIndex"&&(r=r===""?null:r==="true");try{await c.patchSeo({type:t,key:i,field:d,value:r}),n.classList.add("saved-flash"),setTimeout(()=>n.classList.remove("saved-flash"),900);let p=this.data.rows.find(f=>`${f.type}:${f.key}`===e.dataset.ref);p&&d==="seo.metaTitle"&&(p.metaTitle=r||""),p&&d==="seo.metaDescription"&&(p.metaDescription=r||"")}catch(p){l(p.fields?.[n.dataset.field]||p.message,"err")}};n.tagName==="SELECT"?n.addEventListener("change",o):(n.addEventListener("blur",o),n.addEventListener("input",()=>this.updateCount(e,n)))})})}updateCount(e,t){if(t.dataset.field==="seo.metaTitle"){let i=e.querySelector('[data-count="title"]');i&&(i.textContent=`${t.value.length}/60`,i.classList.toggle("bad",t.value.length<30||t.value.length>60))}else if(t.dataset.field==="seo.metaDescription"){let i=e.querySelector('[data-count="desc"]');i&&(i.textContent=`${t.value.length}/160`,i.classList.toggle("bad",t.value.length&&(t.value.length<120||t.value.length>160)))}}};var V=["new","contacted","won","lost"],K={seo:"SEO",meta:"Meta Ads",email:"Email Marketing","ai-seo":"AI SEO","chatgpt-ads":"ChatGPT / AI Ads",software:"Custom Software",multi:"Multi-channel"},Z=/unsubscribe|wa\.me|t\.me\/|whats\s?app|telegram|jackpot|casino|crypto|bitcoin|https?:\/\/|www\.|\b\d{1,3}%\s*off\b|done-for-you|backlinks?\b/i;function $(s){return Z.test(`${s.brief||""} ${s.name||""} ${s.email||""}`)}var C=class{constructor(e){this.root=e,this.showPromos=!1}async mount(){this.root.innerHTML='<div class="loading">Loading\u2026</div>';let e;try{e=await c.listLeads()}catch(t){this.root.innerHTML=`<div class="empty"><h2>Couldn't load leads</h2><p class="muted">${a(t.message)}</p></div>`;return}this._all=e.leads||[],this.render(),this.wire()}render(){let e=this._all.filter($),t=this._all.filter(d=>!$(d)),i=this.showPromos?this._all:t,n=t.filter(d=>d.status==="new").length,o=e.length?`<button class="btn btn-ghost" id="toggle-promos">${this.showPromos?"Hide":"Show"} ${e.length} promotion${e.length===1?"":"s"}</button>`:"";this.root.innerHTML=`
      <header class="page-head">
      ...
      ${i.length?this.table(i):'<div class="empty"><h2>No leads yet</h2><p class="muted">Booking form submissions will appear here.</p></div>'}
    `}table(e){return`<div class="table-scroll"><table class="grid-table leads-table">
      ...
      ${e.map(t=>`<tbody class="lead-group${$(t)?" is-promo":""}">
        <tr>
          <td class="muted small nowrap">${a(M(t.createdAt))}<div class="lead-time">${a(N(t.createdAt))}</div></td>
          <td><strong>${a(t.name)}</strong>${t.company?`<div class="muted small">${a(t.company)}</div>`:""}${$(t)?'<span class="pill pill-mute">promo</span>':""}</td>
          <td><a class="url" href="mailto:${a(t.email)}">${a(t.email)}</a></td>
          <td>${a(K[t.service]||t.service||"\u2014")}</td>
          ...
          <td><select class="input input-sm" data-status="${a(t._id)}">${V.map(i=>`<option value="${i}" ${t.status===i?"selected":""}>${i}</option>`).join("")}</select></td>
        </tr>${t.brief?`<tr class="lead-msg-row"><td colspan="7"><p class="lead-msg" title="${a(t.brief)}">${a(t.brief)}</p></td></tr>`:""}
      </tbody>`).join("")}
      </table></div>`}wire(){this.root.querySelectorAll("[data-status]").forEach(e=>{e.addEventListener("change",async()=>{try{await c.patchLead({id:e.dataset.status,status:e.value}),l("Updated.")}catch(t){l(t.message,"err")}})}),this.root.querySelector("#toggle-promos")?.addEventListener("click",()=>{this.showPromos=!this.showPromos,this.render(),this.wire()}),this.root.querySelector("#csv")?.addEventListener("click",()=>this.exportCsv())}exportCsv(){let e=this._all||[],t=["createdAt","name","email","company","role","service","timeSlot","status","emailSent","isPromo","brief"],i=[t.join(",")].concat(e.map(d=>t.map(r=>{let p=r==="isPromo"?$(d)?"yes":"no":d[r];return`"${String(p??"").replace(/"/g,'""')}"`}).join(","))).join(`
`),n=new Blob([i],{type:"text/csv"}),o=document.createElement("a");o.href=URL.createObjectURL(n),o.download="davnoot-leads.csv",o.click(),URL.revokeObjectURL(o.href)}};var R=class{constructor(e){this.root=e}async mount(){this.root.innerHTML='<div class="loading">Loading\u2026</div>',await this.load()}async load(){let e;try{e=await c.listRedirects()}catch(t){this.root.innerHTML=`<div class="empty"><h2>Couldn't load redirects</h2><p class="muted">${a(t.message)}</p></div>`;return}this.render(e.redirects),this.wire()}render(e){this.root.innerHTML=`
    ...
    `}table(e){return`<div class="table-scroll"><table class="grid-table">
      ...
      </tbody></table></div>`}wire(){this.root.querySelector("#add")?.addEventListener("click",async()=>{let e=this.root.querySelector("#src").value.trim(),t=this.root.querySelector("#dst").value.trim(),i=Number(this.root.querySelector("#code").value);try{await c.createRedirect({source:e,destination:t,status:i}),l("Redirect added."),this.load()}catch(n){l(n.fields?.source||n.fields?.destination||n.message,"err")}}),this.root.querySelectorAll("[data-del]").forEach(e=>{e.addEventListener("click",async()=>{if(await u(`Delete the redirect from ${e.dataset.del}?`,{confirmLabel:"Delete",danger:!0}))try{await c.deleteRedirect(e.dataset.del),l("Deleted."),this.load()}catch(t){l(t.message,"err")}})})}};var D=class{constructor(e){this.root=e}async mount(){this.root.innerHTML='<div class="loading">Loading\u2026</div>';let e;try{e=await c.getSettings()}catch(t){this.root.innerHTML=`<div class="empty"><h2>Couldn't load settings</h2><p class="muted">${a(t.message)}</p></div>`;return}this.eff=e.effective,this.render(e.effective),this.wire()}render(e){this.root.innerHTML=`
      <header class="page-head">
    ...
    `}wire(){this.root.querySelector("#save")?.addEventListener("click",async()=>{let e={};this.root.querySelectorAll("[data-key]").forEach(t=>{J(e,t.dataset.key,t.value)});try{await c.saveSettings(e),l("Settings saved.")}catch(t){l(t.message,"err")}})}};function m(s,e,t,i){let n=i?`<textarea class="input" rows="3" data-key="${s}">${a(t||"")}</textarea>`:`<input class="input" data-key="${s}" value="${a(t||"")}" />`;return`<div class="field"><label>${a(e)}</label>${n}</div>`}function J(s,e,t){let i=e.split("."),n=s;for(let o=0;o<i.length-1;o++)(!n[i[o]]||typeof n[i[o]]!="object")&&(n[i[o]]={}),n=n[i[o]];n[i[i.length-1]]=t}var A=class{constructor(e){this.root=e}async mount(){this.root.innerHTML=`
      <div class="empty">
        <h2>Page not found</h2>
        <p class="muted">Nothing lives at <code>${a(location.pathname)}</code>.</p>
        <p><a class="btn btn-dark" href="/admin">Back to overview</a></p>
      </div>`}};var v=P("#app"),w=null;function X(){let s=location.pathname.replace(/^\/admin\/?/,"").split("/").filter(Boolean);if(!s.length)return{view:"overview"};switch(s[0]){case"pages":return s[1]?{view:"page-editor",key:decodeURIComponent(s.slice(1).join("/"))}:{view:"pages"};case"new":return{view:"page-editor",key:"new"};case"seo":return{view:"seo"};case"leads":return{view:"leads"};case"redirects":return{view:"redirects"};case"settings":return{view:"settings"};default:return{view:"404"}}}var I={overview:()=>new L(v),pages:()=>new T(v),"page-editor":s=>new E(v,{key:s.key}),seo:()=>new x(v),leads:()=>new C(v),redirects:()=>new R(v),settings:()=>new D(v),404:()=>new A(v)};async function U(){w?.destroy?.();let s=X();w=(I[s.view]||I[404])(s),ee(s.view);try{await w.mount()}catch(e){v.innerHTML=`<div class="empty"><h2>Something went wrong</h2><p>${te(e.message)}</p></div>`}}function ee(s){for(let e of document.querySelectorAll(".admin-nav a[data-view]"))e.classList.toggle("active",e.dataset.view===s||s==="page-editor"&&e.dataset.view==="pages")}function te(s){let e=document.createElement("div");return e.textContent=String(s??""),e.innerHTML}document.addEventListener("click",async s=>{let e=s.target.closest("a");if(!e)return;let t=e.getAttribute("href");!t?.startsWith("/admin")||e.target==="_blank"||(s.preventDefault(),!(w?.autosave?.hasUnsavedChanges?.()&&!await u("You have unsaved changes. Leave anyway?",{confirmLabel:"Leave",danger:!0}))&&(history.pushState({},"",t),U()))});window.addEventListener("popstate",U);P("#logout")?.addEventListener("click",async()=>{w?.autosave?.hasUnsavedChanges?.()&&!await u("You have unsaved changes. Sign out anyway?",{confirmLabel:"Sign out",danger:!0})||(await c.logout(),location.href="/seoteam/login")});U();})();
