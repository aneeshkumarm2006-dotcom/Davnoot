/*srchash:0ebc4467ff67424b2adfbba7aaf6b11730822662818d9d4a669add1331448f47*/
(()=>{var i=a=>String(a??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"),D=(a,e=document)=>e.querySelector(a);function z(a){if(!a)return"\u2014";let e=new Date(a);return Number.isNaN(e.getTime())?"\u2014":e.toLocaleDateString("en-CA",{year:"numeric",month:"short",day:"numeric"})}function q(a){if(!a)return"\u2014";let e=new Date(a);return Number.isNaN(e.getTime())?"\u2014":e.toLocaleDateString("en-US",{timeZone:"America/Toronto",month:"short",day:"numeric",year:"numeric"})}function P(a){if(!a)return"";let e=new Date(a);return Number.isNaN(e.getTime())?"":e.toLocaleTimeString("en-US",{timeZone:"America/Toronto",hour:"numeric",minute:"2-digit",hour12:!0})}function w(a){if(!a)return"\u2014";let e=new Date(a),t=Math.round((Date.now()-e.getTime())/6e4);if(t<1)return"just now";if(t<60)return`${t}m ago`;let s=Math.round(t/60);if(s<24)return`${s}h ago`;let n=Math.round(s/24);return n<30?`${n}d ago`:z(a)}function u(a,{confirmLabel:e="Confirm",danger:t=!1}={}){return new Promise(s=>{let n=document.createElement("div");n.className="modal-backdrop",n.innerHTML=`
    ...
    `}table(e,t){return`<div class="table-scroll"><table class="grid-table">
      ...
      <tbody>${e.map(s=>`
        <tr>
          ...
          <td><span class="pill pill-${W(s.status)}">${i(s.status)}</span></td>
    ...
    `}group(e,t){return`<div class="field-group"><h3>${i(e)}</h3>
      ...
      <label>${i(e.label)}</label>
      <input class="input" data-slot="${i(e.key)}" id="${s}" value="${i(Z(t))}" />
...
${t}`,{confirmLabel:"Restore latest"}))try{await c.restoreRevision(this.key,e.revisions[0].version),r("Restored into draft."),this.mount()}catch(s){r(s.message,"err")}}reloadPreview(){clearTimeout(this._pv),this._pv=setTimeout(()=>{let e=this.root.querySelector("#preview");e&&(e.src=e.src)},400)}setStatus(e,t){let s=this.root.querySelector("#status");s&&(s.textContent=e,s.className="editor-status "+(t||""))}renderComposed(){this.root.innerHTML=`
      ...
      <div class="editor-split">
        <div class="editor-fields">
          <div class="section-palette">
            ...
            ${Y.map(e=>`<button class="btn btn-ghost btn-add-section" data-type="${e}">+ ${i(y[e].label)}</button>`).join("")}
          </div>
      ...
      </div>`,this.wireComposed()}sectionCard(e,t){let s=y[e.type];if(!s)return"";let n=s.fields.map(l=>this.fieldControl(`s${t}.${l.key}`,l,e.fields?.[l.key])).join(""),o=s.item?`<div class="section-items"><div class="muted small" style="margin:8px 0 4px">${i(s.item.label)}s</div>
          ${(e.items||[]).map((l,d)=>`<div class="section-item">
            <div class="section-item-head"><span class="muted small">${i(s.item.label)} ${d+1}</span><button class="btn-icon btn-del-item" data-s="${t}" data-i="${d}" title="Remove">\u2715</button></div>
            ${s.item.fields.map(h=>this.fieldControl(`s${t}.i${d}.${h.key}`,h,l?.[h.key])).join("")}
          </div>`).join("")}
...
`).map(G=>G.trim()).filter(Boolean):h=e.value,o!=null?((g=d.items||(d.items=[]))[I=+o]||(g[I]={}),d.items[+o][l]=h):(d.fields||(d.fields={}))[l]=h}touchComposed(){this.dirty=!0,this.setStatus("Unsaved\u2026","dirty"),clearTimeout(this._debounce),this._debounce=setTimeout(()=>this.saveComposed(),900)}async saveComposed(){if(!this.saving){this.saving=!0,this.setStatus("Saving\u2026","dirty");try{let e={title:this.page.draft?.title||this.key,sections:this.sections},t=await c.savePageDraft(this.key,e,this.page.version);this.page.version=t.version,this.dirty=!1,this.setStatus("Saved to draft","saved"),this.reloadPreview()}catch(e){e.status===409?(this.setStatus("Someone else saved \u2014 reload","dirty"),r("This page changed elsewhere. Reload to continue.","err")):(this.setStatus("Save failed","dirty"),r(e.message,"err"))}finally{this.saving=!1}}}renderNew(){this.root.innerHTML=`
      ...
      <div class="table-controls" style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px">
        ${H.map(t=>`<button class="btn btn-sm ${this.filter===t.id?"btn-dark":"btn-ghost"}" data-filter="${t.id}">${i(t.label)}</button>`).join("")}
      </div>
    ...
    </tr>`}wire(){this.root.querySelectorAll("[data-filter]").forEach(e=>e.addEventListener("click",()=>{this.filter=e.dataset.filter,this.render(),this.wire()})),this.root.querySelectorAll("tr[data-ref]").forEach(e=>{let[t,s]=e.dataset.ref.split(/:(.+)/);e.querySelectorAll("[data-field]").forEach(n=>{let o=async()=>{let l=n.dataset.field,d=n.value;l==="seo.robotsIndex"&&(d=d===""?null:d==="true");try{await c.patchSeo({type:t,key:s,field:l,value:d}),n.classList.add("saved-flash"),setTimeout(()=>n.classList.remove("saved-flash"),900);let h=this.data.rows.find(g=>`${g.type}:${g.key}`===e.dataset.ref);h&&l==="seo.metaTitle"&&(h.metaTitle=d||""),h&&l==="seo.metaDescription"&&(h.metaDescription=d||"")}catch(h){r(h.fields?.[n.dataset.field]||h.message,"err")}};n.tagName==="SELECT"?n.addEventListener("change",o):(n.addEventListener("blur",o),n.addEventListener("input",()=>this.updateCount(e,n)))})})}updateCount(e,t){if(t.dataset.field==="seo.metaTitle"){let s=e.querySelector('[data-count="title"]');s&&(s.textContent=`${t.value.length}/60`,s.classList.toggle("bad",t.value.length<30||t.value.length>60))}else if(t.dataset.field==="seo.metaDescription"){let s=e.querySelector('[data-count="desc"]');s&&(s.textContent=`${t.value.length}/160`,s.classList.toggle("bad",t.value.length&&(t.value.length<120||t.value.length>160)))}}};function _({value:a,options:e,id:t="",cls:s="",ariaLabel:n=""}){let o=e.find(l=>l.value===a)||e[0]||{label:""};return`<div class="cdrop${s?" "+s:""}" data-cdrop${t?` data-id="${i(t)}"`:""} data-value="${i(a??"")}">
  ...
  </div>`}function U(a,e){a.classList.toggle("is-open",e);let t=a.querySelector(".cdrop-btn"),s=a.querySelector(".cdrop-menu");t&&t.setAttribute("aria-expanded",e?"true":"false"),s&&(s.hidden=!e)}function M(){document.querySelectorAll(".cdrop.is-open").forEach(a=>U(a,!1))}var j=!1;function ee(){j||(j=!0,document.addEventListener("click",a=>{a.target.closest(".cdrop")||M()}),document.addEventListener("keydown",a=>{a.key==="Escape"&&M()}))}function O(a,e){ee(),a.addEventListener("click",t=>{let s=t.target.closest(".cdrop-btn");if(s&&a.contains(s)){let o=s.closest(".cdrop"),l=!o.classList.contains("is-open");M(),U(o,l);return}let n=t.target.closest(".cdrop-opt");if(n&&a.contains(n)){let o=n.closest(".cdrop"),l=n.dataset.val;U(o,!1),l!==o.dataset.value&&(o.dataset.value=l,o.querySelector(".cdrop-label").textContent=n.textContent,o.querySelectorAll(".cdrop-opt").forEach(d=>d.classList.toggle("is-selected",d===n)),e?.(o.dataset.id||"",l,o))}})}var te=["new","contacted","won","lost"],se=a=>a&&a[0].toUpperCase()+a.slice(1),ae={seo:"SEO",meta:"Meta Ads",email:"Email Marketing","ai-seo":"AI SEO","chatgpt-ads":"ChatGPT / AI Ads",software:"Custom Software",multi:"Multi-channel"},B={},ie=[{key:"inbox",label:"Inbox"},{key:"spam",label:"Spam"},{key:"blocked",label:"Blocked"}],T=class a{constructor(e){this.root=e,this.tab="inbox"}async mount(){this.root.innerHTML='<div class="loading">Loading\u2026</div>';let e;try{e=await c.listLeads()}catch(t){this.root.innerHTML=`<div class="empty"><h2>Couldn't load leads</h2><p class="muted">${i(t.message)}</p></div>`;return}this._all=e.leads||[],this._blocked=e.blocked||[],B=Object.fromEntries((e.categories||[]).map(t=>[t.key,t.label])),this.render(),this.wire()}counts(){let e=this._all.filter(t=>!t.spam);return{inbox:e.length,spam:this._all.length-e.length,blocked:this._blocked.length,newReal:e.filter(t=>t.status==="new").length}}render(){let e=this.counts(),t=this.tab==="blocked"?this._blocked:this._all.filter(o=>this.tab==="spam"?o.spam:!o.spam),s={inbox:`${e.newReal} new \xB7 ${e.inbox} real ${e.inbox===1?"lead":"leads"}. Captured even when the email fails.`,spam:"Caught by the filter and never emailed. Nothing here was deleted \u2014 if one is real, put it back.",blocked:"Rejected before reaching the inbox. Kept 30 days so a wrong call can be spotted, then swept automatically."}[this.tab],n=this.tab==="spam"&&e.spam?'<button class="btn btn-ghost" id="purge">Delete all spam</button>':"";this.root.innerHTML=`
      <header class="page-head">
        <div><h1>Leads</h1><p class="muted">${i(s)}</p></div>
        <div class="page-actions">${n}<button class="btn btn-ghost" id="csv">Export CSV</button></div>
      </header>
      <nav class="lead-tabs">
        ${ie.map(o=>`<button type="button" class="lead-tab${this.tab===o.key?" is-active":""}" data-tab="${o.key}">${o.label}<span class="lead-tab-n">${e[o.key]}</span></button>`).join("")}
      </nav>
      ${t.length?this.tab==="blocked"?this.blockedTable(t):this.table(t):this.emptyState()}
    `}emptyState(){let e={inbox:["No leads yet","Booking form submissions will appear here."],spam:["Nothing in spam","Submissions the filter catches will collect here instead of your inbox."],blocked:["Nothing blocked","Rejected submissions appear here for 30 days."]}[this.tab];return`<div class="empty"><h2>${e[0]}</h2><p class="muted">${e[1]}</p></div>`}static why(e){return!e.spamReasons||!e.spamReasons.length?"":`<div class="lead-why">${i(e.spamReasons.join(" \xB7 "))}${e.spamScore!=null?` <span class="muted">(score ${i(String(e.spamScore))})</span>`:""}</div>`}static categoryPill(e){return e.spamCategory?`<span class="pill pill-spam">${i(B[e.spamCategory]||e.spamCategory)}</span>`:""}table(e){let t=this.tab==="spam";return`<div class="table-scroll"><table class="grid-table leads-table">
      <thead><tr><th>When (ET)</th><th>Name</th><th>Email</th><th>Service</th><th>Slot</th><th>Email</th><th>${t?"Actions":"Status"}</th></tr></thead>
      ${e.map(s=>`<tbody class="lead-group${s.spam?" is-promo":""}">
        <tr>
          <td class="muted small nowrap">${i(q(s.createdAt))}<div class="lead-time">${i(P(s.createdAt))}</div></td>
          <td><strong>${i(s.name)}</strong>${s.company?`<div class="muted small">${i(s.company)}</div>`:""}${a.categoryPill(s)}</td>
          <td><a class="url" href="mailto:${i(s.email)}">${i(s.email)}</a></td>
          <td>${i(ae[s.service]||s.service||"\u2014")}</td>
          <td class="small nowrap">${i(s.timeSlot||"\u2014")}</td>
          <td>${s.spam?'<span class="pill pill-mute">held</span>':s.emailSent?'<span class="pill pill-ok">sent</span>':'<span class="pill pill-warn">failed</span>'}</td>
          <td>
            ${t?"":_({id:s._id,value:s.status||"new",cls:"cdrop-sm",ariaLabel:"Lead status",options:te.map(n=>({value:n,label:se(n)}))})}
            <button type="button" class="lead-promo-btn" data-spam="${i(s._id)}" data-to="${s.spam?"false":"true"}">${s.spam?"Not spam":"Mark as spam"}</button>
            ${t?`<button type="button" class="lead-promo-btn is-danger" data-del="${i(s._id)}">Delete</button>`:""}
          </td>
        </tr>${s.brief||s.spamReasons?.length?`<tr class="lead-msg-row"><td colspan="7">${s.brief?`<p class="lead-msg" title="${i(s.brief)}">${i(s.brief)}</p>`:""}${a.why(s)}</td></tr>`:""}
      </tbody>`).join("")}
      ...
      ${e.map(t=>`<tbody class="lead-group is-promo">
        <tr>
          <td class="muted small nowrap">${i(q(t.createdAt))}<div class="lead-time">${i(P(t.createdAt))}</div></td>
          <td><strong>${i(t.name)}</strong>${t.company?`<div class="muted small">${i(t.company)}</div>`:""}${a.categoryPill(t)}</td>
          <td class="small">${i(t.email)}</td>
          <td class="small">${i((t.spamReasons||[]).join(" \xB7 "))} <span class="muted">(${i(String(t.spamScore??"\u2014"))})</span></td>
        </tr>${t.brief?`<tr class="lead-msg-row"><td colspan="4"><p class="lead-msg" title="${i(t.brief)}">${i(t.brief)}</p></td></tr>`:""}
      </tbody>`).join("")}
      </table></div>`}redraw(){this.render(),this.wire()}wire(){this.root.querySelectorAll("[data-tab]").forEach(e=>{e.addEventListener("click",()=>{this.tab=e.dataset.tab,this.redraw()})}),O(this.root,async(e,t)=>{try{await c.patchLead({id:e,status:t}),r("Status updated.")}catch(s){r(s.message,"err")}}),this.root.querySelectorAll("[data-spam]").forEach(e=>{e.addEventListener("click",async()=>{let t=e.dataset.spam,s=e.dataset.to==="true";try{await c.patchLead({id:t,spam:s,spamCategory:s?"manual":void 0});let n=this._all.find(o=>o._id===t);n&&(n.spam=s,n.spamCategory=s?"manual":null,s||(n.spamReasons=[],n.spamScore=null)),r(s?"Moved to spam.":"Moved back to the inbox."),this.redraw()}catch(n){r(n.message,"err")}})}),this.root.querySelectorAll("[data-del]").forEach(e=>{e.addEventListener("click",async()=>{let t=e.dataset.del;if(confirm("Delete this submission permanently?"))try{await c.deleteLeads({ids:[t]}),this._all=this._all.filter(s=>s._id!==t),r("Deleted."),this.redraw()}catch(s){r(s.message,"err")}})}),this.root.querySelector("#purge")?.addEventListener("click",async()=>{let e=this.counts().spam;if(confirm(`Permanently delete all ${e} spam submission${e===1?"":"s"}? This cannot be undone.`))try{let t=await c.deleteLeads({purge:"spam"});this._all=this._all.filter(s=>!s.spam),r(`Deleted ${t.deleted} submission${t.deleted===1?"":"s"}.`),this.redraw()}catch(t){r(t.message,"err")}}),this.root.querySelector("#csv")?.addEventListener("click",()=>this.exportCsv())}exportCsv(){let e=this._all||[],t=["createdAt","name","email","company","role","service","timeSlot","status","emailSent","spam","spamCategory","spamScore","brief"],s=[t.join(",")].concat(e.map(l=>t.map(d=>{let h=d==="spam"?l.spam?"yes":"no":l[d];return`"${String(h??"").replace(/"/g,'""')}"`}).join(","))).join(`
`),n=new Blob([s],{type:"text/csv"}),o=document.createElement("a");o.href=URL.createObjectURL(n),o.download="davnoot-leads.csv",o.click(),URL.revokeObjectURL(o.href)}};var C=class{constructor(e){this.root=e}async mount(){this.root.innerHTML='<div class="loading">Loading\u2026</div>',await this.load()}async load(){let e;try{e=await c.listRedirects()}catch(t){this.root.innerHTML=`<div class="empty"><h2>Couldn't load redirects</h2><p class="muted">${i(t.message)}</p></div>`;return}this.render(e.redirects),this.wire()}render(e){this.root.innerHTML=`
    ...
    `}table(e){return`<div class="table-scroll"><table class="grid-table">
      ...
      </tbody></table></div>`}wire(){this.root.querySelector("#add")?.addEventListener("click",async()=>{let e=this.root.querySelector("#src").value.trim(),t=this.root.querySelector("#dst").value.trim(),s=Number(this.root.querySelector("#code").value);try{await c.createRedirect({source:e,destination:t,status:s}),r("Redirect added."),this.load()}catch(n){r(n.fields?.source||n.fields?.destination||n.message,"err")}}),this.root.querySelectorAll("[data-del]").forEach(e=>{e.addEventListener("click",async()=>{if(await u(`Delete the redirect from ${e.dataset.del}?`,{confirmLabel:"Delete",danger:!0}))try{await c.deleteRedirect(e.dataset.del),r("Deleted."),this.load()}catch(t){r(t.message,"err")}})})}};var R=class{constructor(e){this.root=e}async mount(){this.root.innerHTML='<div class="loading">Loading\u2026</div>';let e;try{e=await c.getSettings()}catch(t){this.root.innerHTML=`<div class="empty"><h2>Couldn't load settings</h2><p class="muted">${i(t.message)}</p></div>`;return}this.eff=e.effective,this.render(e.effective),this.wire()}render(e){this.root.innerHTML=`
      <header class="page-head">
    ...
    `}wire(){this.root.querySelector("#save")?.addEventListener("click",async()=>{let e={};this.root.querySelectorAll("[data-key]").forEach(t=>{ne(e,t.dataset.key,t.value)});try{await c.saveSettings(e),r("Settings saved.")}catch(t){r(t.message,"err")}})}};function m(a,e,t,s){let n=s?`<textarea class="input" rows="3" data-key="${a}">${i(t||"")}</textarea>`:`<input class="input" data-key="${a}" value="${i(t||"")}" />`;return`<div class="field"><label>${i(e)}</label>${n}</div>`}function ne(a,e,t){let s=e.split("."),n=a;for(let o=0;o<s.length-1;o++)(!n[s[o]]||typeof n[s[o]]!="object")&&(n[s[o]]={}),n=n[s[o]];n[s[s.length-1]]=t}var A=class{constructor(e){this.root=e}async mount(){this.root.innerHTML=`
      <div class="empty">
        <h2>Page not found</h2>
        <p class="muted">Nothing lives at <code>${i(location.pathname)}</code>.</p>
        <p><a class="btn btn-dark" href="/admin">Back to overview</a></p>
      </div>`}};var v=D("#app"),$=null;function oe(){let a=location.pathname.replace(/^\/admin\/?/,"").split("/").filter(Boolean);if(!a.length)return{view:"overview"};switch(a[0]){case"pages":return a[1]?{view:"page-editor",key:decodeURIComponent(a.slice(1).join("/"))}:{view:"pages"};case"new":return{view:"page-editor",key:"new"};case"seo":return{view:"seo"};case"leads":return{view:"leads"};case"redirects":return{view:"redirects"};case"settings":return{view:"settings"};default:return{view:"404"}}}var F={overview:()=>new S(v),pages:()=>new L(v),"page-editor":a=>new E(v,{key:a.key}),seo:()=>new x(v),leads:()=>new T(v),redirects:()=>new C(v),settings:()=>new R(v),404:()=>new A(v)};async function N(){$?.destroy?.();let a=oe();$=(F[a.view]||F[404])(a),le(a.view);try{await $.mount()}catch(e){v.innerHTML=`<div class="empty"><h2>Something went wrong</h2><p>${re(e.message)}</p></div>`}}function le(a){for(let e of document.querySelectorAll(".admin-nav a[data-view]"))e.classList.toggle("active",e.dataset.view===a||a==="page-editor"&&e.dataset.view==="pages")}function re(a){let e=document.createElement("div");return e.textContent=String(a??""),e.innerHTML}document.addEventListener("click",async a=>{let e=a.target.closest("a");if(!e)return;let t=e.getAttribute("href");!t?.startsWith("/admin")||e.target==="_blank"||(a.preventDefault(),!($?.autosave?.hasUnsavedChanges?.()&&!await u("You have unsaved changes. Leave anyway?",{confirmLabel:"Leave",danger:!0}))&&(history.pushState({},"",t),N()))});window.addEventListener("popstate",N);D("#logout")?.addEventListener("click",async()=>{$?.autosave?.hasUnsavedChanges?.()&&!await u("You have unsaved changes. Sign out anyway?",{confirmLabel:"Sign out",danger:!0})||(await c.logout(),location.href="/seoteam/login")});N();})();
