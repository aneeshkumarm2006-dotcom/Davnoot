    ...
    `}table(e,t){return`<div class="table-scroll"><table class="grid-table">
      ...
      <tbody>${e.map(s=>`
        <tr>
          ...
          <td><span class="pill pill-${X(s.status)}">${i(s.status)}</span></td>
      ...
      <div class="editor-split">
        <div class="editor-fields">
          <div class="section-palette">
            ...
            ${ee.map(e=>`<button class="btn btn-ghost btn-add-section" data-type="${e}">+ ${i(k[e].label)}</button>`).join("")}
          </div>
        ...
        <div class="editor-preview">
          ...
          ${(e.items||[]).map((l,r)=>`<div class="section-item">
            ...
            ${s.item.fields.map(c=>this.fieldControl(`s${t}.i${r}.${c.key}`,c,l?.[c.key])).join("")}
          </div>`).join("")}
...
`).map(h=>h.trim()).filter(Boolean):c=e.value,o!=null?((v=r.items||(r.items=[]))[g=+o]||(v[g]={}),r.items[+o][l]=c):(r.fields||(r.fields={}))[l]=c}touchComposed(){this.dirty=!0,this.setStatus("Unsaved\u2026","dirty"),clearTimeout(this._debounce),this._debounce=setTimeout(()=>this.saveComposed(),900)}async saveComposed(){if(!this.saving){this.saving=!0,this.setStatus("Saving\u2026","dirty");try{let e={title:this.page.draft?.title||this.key,sections:this.sections},t=await p.savePageDraft(this.key,e,this.page.version);this.page.version=t.version,this.dirty=!1,this.setStatus("Saved to draft","saved"),this.reloadPreview()}catch(e){e.status===409?(this.setStatus("Someone else saved \u2014 reload","dirty"),d("This page changed elsewhere. Reload to continue.","err")):(this.setStatus("Save failed","dirty"),d(e.message,"err"))}finally{this.saving=!1}}}renderNew(){this.root.innerHTML=`
      ...
      </div>`;let e=this.root.querySelector("#np-slug");e.addEventListener("input",()=>{this.root.querySelector("#np-preview").textContent=e.value||"slug"}),this.root.querySelector("#np-create").addEventListener("click",async()=>{try{let t=await p.createPage({title:this.root.querySelector("#np-title").value,slug:e.value.trim(),kind:this.root.querySelector("#np-kind").value});d("Page created."),history.pushState({},"",`/admin/pages/${encodeURIComponent(t.key)}`),window.dispatchEvent(new PopStateEvent("popstate"))}catch(t){d(t.fields?.slug||t.message,"err")}})}};function te(a){var t;let e={};for(let s of a)(e[t=s.group]||(e[t]=[])).push(s);return e}function se(a){let e={};for(let[t,s]of Object.entries(a?.seo||{}))s!=null&&(e["seo."+t]=s);for(let t of a?.sections||[])for(let[s,n]of Object.entries(t.fields||{}))n!=null&&(e[s]=n);return e}function ae(a,e){let t={},s={};for(let[o,l]of Object.entries(a))o.startsWith("seo.")?t[o.slice(4)]=l:s[o]=l;let n={title:e.draft?.title||"",sections:[{id:"overlay",source:"base",fields:s}]};return Object.keys(t).length&&(n.seo=t),n}function ie(a){return String(a||"").replace(/<[^>]+>/g,"")}function ne(a){let e=k[a],t={id:a+"-"+Math.random().toString(36).slice(2,8),type:a,source:"library",fields:{}};return e?.item&&(t.items=[{}]),t}function oe(a){return{id:a.id,type:a.type,source:a.source||"library",hidden:!!a.hidden,fields:a.fields||{},items:Array.isArray(a.items)?a.items:void 0}}var W=[{id:"all",label:"All"},{id:"no-desc",label:"Missing description",test:a=>!a.metaDescription},{id:"title-len",label:"Title out of range",test:a=>a.titleLen<30||a.titleLen>60},{id:"noindex",label:"Noindexed",test:a=>a.robotsIndex===!1},{id:"dupe-title",label:"Duplicate title"},{id:"off-canon",label:"Off-site canonical",test:a=>a.canonicalUrl&&!a.canonicalUrl.includes("davnoot.com")}],D=class{constructor(e){this.root=e,this.filter="all"}async mount(){this.root.innerHTML='<div class="loading">Loading\u2026</div>';try{this.data=await p.seoTable()}catch(e){this.root.innerHTML=`<div class="empty"><h2>Couldn't load the SEO table</h2><p class="muted">${i(e.message)}</p></div>`;return}this.markDuplicates(),this.render(),this.wire()}markDuplicates(){let e=new Map;for(let t of this.data.rows){let s=(t.metaTitle||t.title||"").trim().toLowerCase();s&&e.set(s,(e.get(s)||0)+1)}for(let t of this.data.rows){let s=(t.metaTitle||t.title||"").trim().toLowerCase();t._dupeTitle=s&&e.get(s)>1}}rows(){let e=W.find(t=>t.id===this.filter);return this.filter==="dupe-title"?this.data.rows.filter(t=>t._dupeTitle):e?.test?this.data.rows.filter(e.test):this.data.rows}render(){let e=this.rows();this.root.innerHTML=`
      <header class="page-head">
      ...
      <div class="table-controls" style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px">
        ${W.map(t=>`<button class="btn btn-sm ${this.filter===t.id?"btn-dark":"btn-ghost"}" data-filter="${t.id}">${i(t.label)}</button>`).join("")}
      </div>
    ...
    </tr>`}wire(){this.root.querySelectorAll("[data-filter]").forEach(e=>e.addEventListener("click",()=>{this.filter=e.dataset.filter,this.render(),this.wire()})),this.root.querySelectorAll("tr[data-ref]").forEach(e=>{let[t,s]=e.dataset.ref.split(/:(.+)/);e.querySelectorAll("[data-field]").forEach(n=>{let o=async()=>{let l=n.dataset.field,r=n.value;l==="seo.robotsIndex"&&(r=r===""?null:r==="true");try{await p.patchSeo({type:t,key:s,field:l,value:r}),n.classList.add("saved-flash"),setTimeout(()=>n.classList.remove("saved-flash"),900);let c=this.data.rows.find(v=>`${v.type}:${v.key}`===e.dataset.ref);c&&l==="seo.metaTitle"&&(c.metaTitle=r||""),c&&l==="seo.metaDescription"&&(c.metaDescription=r||"")}catch(c){d(c.fields?.[n.dataset.field]||c.message,"err")}};n.tagName==="SELECT"?n.addEventListener("change",o):(n.addEventListener("blur",o),n.addEventListener("input",()=>this.updateCount(e,n)))})})}updateCount(e,t){if(t.dataset.field==="seo.metaTitle"){let s=e.querySelector('[data-count="title"]');s&&(s.textContent=`${t.value.length}/60`,s.classList.toggle("bad",t.value.length<30||t.value.length>60))}else if(t.dataset.field==="seo.metaDescription"){let s=e.querySelector('[data-count="desc"]');s&&(s.textContent=`${t.value.length}/160`,s.classList.toggle("bad",t.value.length&&(t.value.length<120||t.value.length>160)))}}};function I({value:a,options:e,id:t="",cls:s="",ariaLabel:n=""}){let o=e.find(l=>l.value===a)||e[0]||{label:""};return`<div class="cdrop${s?" "+s:""}" data-cdrop${t?` data-id="${i(t)}"`:""} data-value="${i(a??"")}">
  ...
  </div>`}function j(a,e){a.classList.toggle("is-open",e);let t=a.querySelector(".cdrop-btn"),s=a.querySelector(".cdrop-menu");t&&t.setAttribute("aria-expanded",e?"true":"false"),s&&(s.hidden=!e)}function _(){document.querySelectorAll(".cdrop.is-open").forEach(a=>j(a,!1))}var G=!1;function le(){G||(G=!0,document.addEventListener("click",a=>{a.target.closest(".cdrop")||_()}),document.addEventListener("keydown",a=>{a.key==="Escape"&&_()}))}function H(a,e){le(),a.addEventListener("click",t=>{let s=t.target.closest(".cdrop-btn");if(s&&a.contains(s)){let o=s.closest(".cdrop"),l=!o.classList.contains("is-open");_(),j(o,l);return}let n=t.target.closest(".cdrop-opt");if(n&&a.contains(n)){let o=n.closest(".cdrop"),l=n.dataset.val;j(o,!1),l!==o.dataset.value&&(o.dataset.value=l,o.querySelector(".cdrop-label").textContent=n.textContent,o.querySelectorAll(".cdrop-opt").forEach(r=>r.classList.toggle("is-selected",r===n)),e?.(o.dataset.id||"",l,o))}})}var z=["new","contacted","won","lost"],Y=a=>a&&a[0].toUpperCase()+a.slice(1),K={seo:"SEO",meta:"Meta Ads",email:"Email Marketing","ai-seo":"AI SEO","chatgpt-ads":"ChatGPT / AI Ads",software:"Custom Software",multi:"Multi-channel"},Q={},O={},B={},re=[{key:"inbox",label:"Inbox"},{key:"spam",label:"Spam"},{key:"blocked",label:"Blocked"}],q=class a{constructor(e){this.root=e,this.tab="inbox",this.source="all"}async mount(){this.root.innerHTML='<div class="loading">Loading\u2026</div>';let e;try{e=await p.listLeads()}catch(t){this.root.innerHTML=`<div class="empty"><h2>Couldn't load leads</h2><p class="muted">${i(t.message)}</p></div>`;return}this._all=e.leads||[],this._blocked=e.blocked||[],this._sources=e.sources||[],Q=Object.fromEntries((e.categories||[]).map(t=>[t.key,t.label])),O=Object.fromEntries(this._sources.map(t=>[t.key,t.label])),B=Object.fromEntries((e.auditScopes||[]).map(t=>[t.key,t.label])),this.render(),this.wire()}counts(){let e=this._all.filter(t=>!t.spam);return{inbox:e.length,spam:this._all.length-e.length,blocked:this._blocked.length,newReal:e.filter(t=>t.status==="new").length}}visible(){let e=this.tab==="blocked"?this._blocked:this._all.filter(t=>this.tab==="spam"?t.spam:!t.spam);return this.source==="all"?e:e.filter(t=>t.source===this.source)}render(){let e=this.counts(),t=this.visible(),s={inbox:`${e.newReal} new \xB7 ${e.inbox} real ${e.inbox===1?"lead":"leads"}. Captured even when the email fails.`,spam:"Caught by the filter and never emailed. Nothing here was deleted \u2014 if one is real, put it back.",blocked:"Rejected before reaching the inbox. Kept 30 days so a wrong call can be spotted, then swept automatically."}[this.tab],n=this.tab==="spam"&&e.spam?'<button class="btn btn-ghost" id="purge">Delete all spam</button>':"";this.root.innerHTML=`
      <header class="page-head">
        <div><h1>Leads</h1><p class="muted">${i(s)}</p></div>
        <div class="page-actions">${n}<button class="btn btn-ghost" id="csv">Export CSV</button></div>
      </header>
      <nav class="lead-tabs">
        ${re.map(o=>`<button type="button" class="lead-tab${this.tab===o.key?" is-active":""}" data-tab="${o.key}">${o.label}<span class="lead-tab-n">${e[o.key]}</span></button>`).join("")}
      </nav>
      ${this.sourceFilter()}
      ${t.length?this.tab==="blocked"?this.blockedTable(t):this.table(t):this.emptyState()}
    `}sourceFilter(){let e=this.tab==="blocked"?this._blocked:this._all,t=this._sources.filter(n=>e.some(o=>o.source===n.key));if(t.length<2)return"";let s=(n,o)=>`<button type="button" class="lead-src${this.source===n?" is-active":""}" data-src="${i(n)}">${i(o)}</button>`;return`<div class="lead-sources">${s("all","All sources")}${t.map(n=>s(n.key,n.label)).join("")}</div>`}emptyState(){if(this.source!=="all"){let t=O[this.source]||this.source;return`<div class="empty"><h2>Nothing from ${i(t)}</h2><p class="muted">Other sources may still have submissions \u2014 switch back to All sources.</p></div>`}let e={inbox:["No leads yet","Booking calls, site-audit requests, blog teardowns and content reviews all land here."],spam:["Nothing in spam","Submissions the filter catches will collect here instead of your inbox."],blocked:["Nothing blocked","Rejected submissions appear here for 30 days."]}[this.tab];return`<div class="empty"><h2>${e[0]}</h2><p class="muted">${e[1]}</p></div>`}static why(e){return!e.spamReasons||!e.spamReasons.length?"":`<div class="lead-why">${i(e.spamReasons.join(" \xB7 "))}${e.spamScore!=null?` <span class="muted">(score ${i(String(e.spamScore))})</span>`:""}</div>`}static categoryPill(e){return e.spamCategory?`<span class="pill pill-spam">${i(Q[e.spamCategory]||e.spamCategory)}</span>`:""}static sourcePill(e){let t=O[e.source];return t?`<span class="pill pill-src pill-src-${i(e.source)}">${i(t)}</span>`:""}static emailPill(e){return e.emailSent===!0?'<span class="pill pill-ok">sent</span>':e.emailSent===null||e.emailSent===void 0?'<span class="pill pill-mute" title="Worked in the admin inbox \u2014 no notification email is sent for this source">admin only</span>':e.spam?'<span class="pill pill-mute">held</span>':'<span class="pill pill-warn">failed</span>'}static whoLabel(e){if(e.name)return`<strong>${i(e.name)}</strong>`;if(e.website){let t=e.website.replace(/^https?:\/\//i,"").replace(/^www\./i,"");return`<strong>${i(t.split("/")[0])}</strong>`}return e.email?`<strong>${i(e.email)}</strong>`:'<span class="muted">\u2014</span>'}static sub(e){return e.company?`<div class="muted small">${i(e.company)}</div>`:""}static wants(e){return K[e.service]||B[e.auditScope]||e.service||"\u2014"}static detail(e){let t=[];e.brief&&t.push(`<p class="lead-msg" title="${i(e.brief)}">${i(e.brief)}</p>`),e.website&&t.push(`<p class="lead-detail"><span class="lead-detail-k">Site</span> <a class="url" href="${i(e.website)}" target="_blank" rel="noopener noreferrer">${i(e.website)}</a>${e.sourceUrl?` <span class="lead-detail-k">Read on</span> <a class="url" href="${i(e.sourceUrl)}" target="_blank" rel="noopener noreferrer">${i(e.sourceUrl)}</a>`:""}</p>`);let s=e.analysis;if(s){let o=[];s.keyword&&o.push(`<span class="lead-detail-k">Keyword</span> ${i(s.keyword)}`),s.score!=null&&o.push(`<span class="lead-detail-k">Scored</span> ${i(String(s.score))}/100`),s.wordCount!=null&&o.push(`<span class="lead-detail-k">Words</span> ${i(String(s.wordCount))}`),o.length&&t.push(`<p class="lead-detail">${o.join(" ")}</p>`)}t.push(a.why(e));let n=t.join("");return n.trim()?n:""}table(e){let t=this.tab==="spam",s=8;return`<div class="table-scroll"><table class="grid-table leads-table">
      ...
      ${e.map(n=>{let o=a.detail(n);return`<tbody class="lead-group${n.spam?" is-promo":""}" data-open="${i(n._id)}">
        <tr>
          ...
          <td>${a.emailPill(n)}</td>
          <td>
            ${t?"":I({id:n._id,value:n.status||"new",cls:"cdrop-sm",ariaLabel:"Lead status",options:z.map(l=>({value:l,label:Y(l)}))})}
      ...
      </div>`;let l=()=>{o.remove(),document.removeEventListener("keydown",r,!0),this._closeDetail=null,n&&document.contains(n)&&n.focus()},r=h=>{h.key==="Escape"&&(o.querySelector(".cdrop.is-open")||l())};o.addEventListener("click",h=>{(h.target===o||h.target.closest('[data-act="close"]'))&&l()}),document.addEventListener("keydown",r,!0),document.body.appendChild(o),this._closeDetail=l;let c=o.querySelector("#ld-notes");c&&(c.value=s.notes||""),o.querySelector(".modal-x").focus(),H(o,async(h,$)=>{try{await p.patchLead({id:h,status:$}),s.status=$,d("Status updated."),this.redraw()}catch(Z){d(Z.message,"err")}});let v=o.querySelector("#ld-save-notes"),g=o.querySelector("#ld-notes-state");c?.addEventListener("input",()=>{g.textContent="Unsaved"}),v?.addEventListener("click",async()=>{v.disabled=!0;try{await p.patchLead({id:s._id,notes:c.value}),s.notes=c.value,g.textContent="Saved.",d("Notes saved.")}catch(h){g.textContent="",d(h.message,"err")}v.disabled=!1}),o.querySelector("[data-spam]")?.addEventListener("click",async h=>{let $=h.currentTarget;await this.setSpam($.dataset.spam,$.dataset.to==="true")&&(l(),this.redraw())}),o.querySelector("[data-del]")?.addEventListener("click",async h=>{confirm("Delete this submission permanently?")&&await this.removeLead(h.currentTarget.dataset.del)&&(l(),this.redraw())})}static detailList(e,t){let s=(r,c)=>c?`<div class="ld-row"><dt>${i(r)}</dt><dd>${c}</dd></div>`:"",n=r=>`<a class="url" href="${i(r)}" target="_blank" rel="noopener noreferrer">${i(r)}</a>`,l=e.spamCategory||(e.spamReasons||[]).length?`${a.categoryPill(e)}${e.spamScore!=null?` <span class="muted small">score ${i(String(e.spamScore))}</span>`:""}${(e.spamReasons||[]).length?`<div class="ld-reasons">${i(e.spamReasons.join(" \xB7 "))}</div>`:""}`:"";return`<dl class="ld">
      ...
      ${s("Role",i(e.role||""))}
      ${s("Phone",e.phone?`<a class="url" href="tel:${i(e.phone.replace(/[^+\d]/g,""))}">${i(e.phone)}</a>`:"")}
      ${s("Service",e.service?i(K[e.service]||e.service):"")}
      ${s("Wants audited",e.auditScope?i(B[e.auditScope]||e.auditScope):"")}
    ...
    </dl>`}static messageBlock(e){return e.brief?`<section class="ld-block"><h3>Message</h3><p class="ld-full">${i(e.brief)}</p></section>`:""}static analysisBlock(e){let t=e.analysis;if(!t)return"";let s=(l,r)=>r?`<div class="ld-row"><dt>${i(l)}</dt><dd>${i(r)}</dd></div>`:"",n=(l,r,c)=>r==null?"":`<div class="ld-row"><dt>${i(l)}</dt><dd>${i(String(r))}${i(c||"")}</dd></div>`,o=(t.failed||[]).length?`<div class="ld-row"><dt>Failing checks</dt><dd><span class="ld-mono">${i(t.failed.join(", "))}</span></dd></div>`:"";return`<section class="ld-block">
      <h3>What they analyzed</h3>
      <dl class="ld">
        ${s("Focus keyword",t.keyword)}
        ${s("SEO title",t.seoTitle)}
        ${s("URL slug",t.slug)}
        ${s("Meta description",t.metaDescription)}
        ${n("Score",t.score,"/100")}
        ${n("Word count",t.wordCount)}
        ${o}
        ${s("Tool language",t.locale==="fr"?"French":t.locale==="en"?"English":t.locale)}
      </dl>
      ${t.content?`<h4 class="ld-sub">Their draft${t.contentTruncated?' <span class="muted small">(truncated at 25,000 characters)</span>':""}</h4>
      <pre class="ld-draft">${i(t.content)}</pre>`:""}
    </section>`}static notesBlock(){return`<section class="ld-block">
      <h3>Private notes</h3>
      ...
      <div class="ld-notes-foot">
        ...
        ${e.spam?"":I({id:e._id,value:e.status||"new",cls:"cdrop-sm",ariaLabel:"Lead status",options:z.map(o=>({value:o,label:Y(o)}))})}
      ...
      <div class="modal-actions">${s}${n}</div>`}closeDetail(){this._closeDetail?.()}destroy(){this.closeDetail()}async setSpam(e,t){try{await p.patchLead({id:e,spam:t,spamCategory:t?"manual":void 0});let s=this._all.find(n=>n._id===e);return s&&(s.spam=t,s.spamCategory=t?"manual":null,t||(s.spamReasons=[],s.spamScore=null)),d(t?"Moved to spam.":"Moved back to the inbox."),!0}catch(s){return d(s.message,"err"),!1}}async removeLead(e){try{return await p.deleteLeads({ids:[e]}),this._all=this._all.filter(t=>t._id!==e),d("Deleted."),!0}catch(t){return d(t.message,"err"),!1}}redraw(){this.render(),this.wire()}wire(){this.root.querySelectorAll("[data-tab]").forEach(e=>{e.addEventListener("click",()=>{this.tab=e.dataset.tab,this.source="all",this.redraw()})}),this.root.querySelectorAll("[data-src]").forEach(e=>{e.addEventListener("click",()=>{this.source=e.dataset.src,this.redraw()})}),H(this.root,async(e,t)=>{try{await p.patchLead({id:e,status:t}),d("Status updated.")}catch(s){d(s.message,"err")}}),this.root.querySelectorAll("tbody[data-open]").forEach(e=>{e.addEventListener("click",t=>{let s=t.target.closest("a, button, .cdrop, input, textarea, select");s&&!s.classList.contains("lead-open")||this.openDetail(e.dataset.open)})}),this.root.querySelectorAll("[data-spam]").forEach(e=>{e.addEventListener("click",async()=>{await this.setSpam(e.dataset.spam,e.dataset.to==="true")&&this.redraw()})}),this.root.querySelectorAll("[data-del]").forEach(e=>{e.addEventListener("click",async()=>{confirm("Delete this submission permanently?")&&await this.removeLead(e.dataset.del)&&this.redraw()})}),this.root.querySelector("#purge")?.addEventListener("click",async()=>{let e=this.counts().spam;if(confirm(`Permanently delete all ${e} spam submission${e===1?"":"s"}? This cannot be undone.`))try{let t=await p.deleteLeads({purge:"spam"});this._all=this._all.filter(s=>!s.spam),d(`Deleted ${t.deleted} submission${t.deleted===1?"":"s"}.`),this.redraw()}catch(t){d(t.message,"err")}}),this.root.querySelector("#csv")?.addEventListener("click",()=>this.exportCsv())}exportCsv(){let e=this._all||[],t=["createdAt","source","name","email","phone","website","sourceUrl","company","role","service","auditScope","keyword","contentScore","timeSlot","status","emailSent","spam","spamCategory","spamScore","brief"],s=(r,c)=>c==="spam"?r.spam?"yes":"no":c==="keyword"?r.analysis?.keyword??"":c==="contentScore"?r.analysis?.score??"":r[c],n=[t.join(",")].concat(e.map(r=>t.map(c=>`"${String(s(r,c)??"").replace(/"/g,'""')}"`).join(","))).join(`
`),o=new Blob([n],{type:"text/csv"}),l=document.createElement("a");l.href=URL.createObjectURL(o),l.download="davnoot-leads.csv",l.click(),URL.revokeObjectURL(l.href)}};var P=class{constructor(e){this.root=e}async mount(){this.root.innerHTML='<div class="loading">Loading\u2026</div>',await this.load()}async load(){let e;try{e=await p.listRedirects()}catch(t){this.root.innerHTML=`<div class="empty"><h2>Couldn't load redirects</h2><p class="muted">${i(t.message)}</p></div>`;return}this.render(e.redirects),this.wire()}render(e){this.root.innerHTML=`
      <header class="page-head">
    ...
    `}table(e){return`<div class="table-scroll"><table class="grid-table">
      ...
      </tbody></table></div>`}wire(){this.root.querySelector("#add")?.addEventListener("click",async()=>{let e=this.root.querySelector("#src").value.trim(),t=this.root.querySelector("#dst").value.trim(),s=Number(this.root.querySelector("#code").value);try{await p.createRedirect({source:e,destination:t,status:s}),d("Redirect added."),this.load()}catch(n){d(n.fields?.source||n.fields?.destination||n.message,"err")}}),this.root.querySelectorAll("[data-del]").forEach(e=>{e.addEventListener("click",async()=>{if(await m(`Delete the redirect from ${e.dataset.del}?`,{confirmLabel:"Delete",danger:!0}))try{await p.deleteRedirect(e.dataset.del),d("Deleted."),this.load()}catch(t){d(t.message,"err")}})})}};var U=class{constructor(e){this.root=e}async mount(){this.root.innerHTML='<div class="loading">Loading\u2026</div>';let e;try{e=await p.getSettings()}catch(t){this.root.innerHTML=`<div class="empty"><h2>Couldn't load settings</h2><p class="muted">${i(t.message)}</p></div>`;return}this.eff=e.effective,this.render(e.effective),this.wire()}render(e){this.root.innerHTML=`
      <header class="page-head">
    ...
    `}wire(){this.root.querySelector("#save")?.addEventListener("click",async()=>{let e={};this.root.querySelectorAll("[data-key]").forEach(t=>{de(e,t.dataset.key,t.value)});try{await p.saveSettings(e),d("Settings saved.")}catch(t){d(t.message,"err")}})}};function b(a,e,t,s){let n=s?`<textarea class="input" rows="3" data-key="${a}">${i(t||"")}</textarea>`:`<input class="input" data-key="${a}" value="${i(t||"")}" />`;return`<div class="field"><label>${i(e)}</label>${n}</div>`}function de(a,e,t){let s=e.split("."),n=a;for(let o=0;o<s.length-1;o++)(!n[s[o]]||typeof n[s[o]]!="object")&&(n[s[o]]={}),n=n[s[o]];n[s[s.length-1]]=t}var N=class{constructor(e){this.root=e}async mount(){this.root.innerHTML=`
      <div class="empty">
        <h2>Page not found</h2>
        <p class="muted">Nothing lives at <code>${i(location.pathname)}</code>.</p>
        <p><a class="btn btn-dark" href="/admin">Back to overview</a></p>
      </div>`}};var f=M("#app"),S=null;function ce(){let a=location.pathname.replace(/^\/admin\/?/,"").split("/").filter(Boolean);if(!a.length)return{view:"overview"};switch(a[0]){case"pages":return a[1]?{view:"page-editor",key:decodeURIComponent(a.slice(1).join("/"))}:{view:"pages"};case"new":return{view:"page-editor",key:"new"};case"seo":return{view:"seo"};case"leads":return{view:"leads"};case"redirects":return{view:"redirects"};case"settings":return{view:"settings"};default:return{view:"404"}}}var V={overview:()=>new C(f),pages:()=>new R(f),"page-editor":a=>new A(f,{key:a.key}),seo:()=>new D(f),leads:()=>new q(f),redirects:()=>new P(f),settings:()=>new U(f),404:()=>new N(f)};async function F(){S?.destroy?.();let a=ce();S=(V[a.view]||V[404])(a),pe(a.view);try{await S.mount()}catch(e){f.innerHTML=`<div class="empty"><h2>Something went wrong</h2><p>${ue(e.message)}</p></div>`}}function pe(a){for(let e of document.querySelectorAll(".admin-nav a[data-view]"))e.classList.toggle("active",e.dataset.view===a||a==="page-editor"&&e.dataset.view==="pages")}function ue(a){let e=document.createElement("div");return e.textContent=String(a??""),e.innerHTML}document.addEventListener("click",async a=>{let e=a.target.closest("a");if(!e)return;let t=e.getAttribute("href");!t?.startsWith("/admin")||e.target==="_blank"||(a.preventDefault(),!(S?.autosave?.hasUnsavedChanges?.()&&!await m("You have unsaved changes. Leave anyway?",{confirmLabel:"Leave",danger:!0}))&&(history.pushState({},"",t),F()))});window.addEventListener("popstate",F);M("#logout")?.addEventListener("click",async()=>{S?.autosave?.hasUnsavedChanges?.()&&!await m("You have unsaved changes. Sign out anyway?",{confirmLabel:"Sign out",danger:!0})||(await p.logout(),location.href="/seoteam/login")});F();})();
